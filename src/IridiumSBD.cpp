...
int IridiumSBD::sleep() {
    ...
    if (ret == ISBD_SUCCESS) power(false);  // power off
    return ret;
}
...
void IridiumSBD::enableRingAlerts(
    bool enable)  // true to enable SBDRING alerts and RING signal pin
{
    this->ringAlertsEnabled = enable;
    if (enable) this->ringAsserted = false;
}
...
int IridiumSBD::getSystemTime(struct tm &tm) {
    ...
    // Latest epoch began at Feb 14, 2025, at 18:14:17 UTC.
    // Iridium periodically resets the SSI epoch before the 32-bit 90ms tick
    // counter overflows (~12.25 years). Previous epoch was May 11, 2014.
    struct tm epoch_start;
    epoch_start.tm_year = 2025 - 1900;
    epoch_start.tm_mon = 2 - 1;
    epoch_start.tm_mday = 14;
    epoch_start.tm_hour = 18;
    epoch_start.tm_min = 14;
    epoch_start.tm_sec = 17;
    ...
    */
    ...
    unsigned long small_ticks =
        ticks_since_epoch - (secs_since_epoch / 90) * 1000;
...
int IridiumSBD::internalBegin() {
    ...
    for (unsigned long start = millis();
         !modemAlive && millis() - start < 1000UL * ISBD_STARTUP_MAX_TIME;) {
        send(F("AT\r"));
...
int IridiumSBD::internalSendReceiveSBD(const char *txTxtMessage,
    ...
    for (unsigned long start = millis();
         millis() - start < 1000UL * this->sendReceiveTimeout;) {
        ...
        if (okToProceed) {
            ...
            if (moCode >= 0 &&
                moCode <= 4)  // this range indicates successful return!
            {
            ...
            else if (moCode == 12 || moCode == 14 ||
                     moCode == 16)  // fatal failure: no retry
            {
...
#if false  // recent research suggest this is not what you should do when just
           // sleeping
...
bool IridiumSBD::waitForATResponse(char *response, int responseSize,
    ...
    if (response) memset(response, 0, responseSize);
...
int IridiumSBD::doSBDRB(uint8_t *rxBuffer, size_t *prxBufferSize) {
    ...
    if (stream.available() < 2) return ISBD_SENDRECEIVE_TIMEOUT;
...
void IridiumSBD::send(FlashString str, bool beginLine, bool endLine) {
    if (beginLine) consoleprint(F(">> "));
    consoleprint(str);
    if (endLine) consoleprint(F("\r\n"));
    stream.print(str);
}
...
void IridiumSBD::diagprint(const char *str) {
    while (*str) ISBDDiagsCallback(this, *str++);
}
...
void IridiumSBD::consoleprint(const char *str) {
    while (*str) ISBDConsoleCallback(this, *str++);
}
...
void IridiumSBD::consoleprint(char c) { ISBDConsoleCallback(this, c); }
